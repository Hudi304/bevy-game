...
pub fn remove_vec3_duplicates(all_filtered_vertices: &Vec<Vec3>, eps: f32) -> Vec<Vec3> {
    ...
    for city_pos in city_points_iter {
        let ord = compare_vec3(prev_city_pos, city_pos, eps);
...
pub fn remove_edges_duplicates(
    all_filtered_vertices: &Vec<(Vec3, f32)>,
    eps: f32
) -> Vec<(Vec3, f32)> {
    let mut city_points_iter = all_filtered_vertices.iter();
    ...
    for road_pos in city_points_iter {
        let ord = compare_vec3(&prev_city_pos.0, &road_pos.0, eps);

        if ord != Ordering::Equal {
            unique_city_positions.push(road_pos.clone());
        }

        prev_city_pos = road_pos;
    }
    unique_city_positions
}
